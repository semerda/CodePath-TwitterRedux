...
/*
    open override func drawText(in rect: CGRect) {
        ...
        let glyphOrigin = CGPoint(rect.origin.x, glyphOriginY)
        
        layoutManager.drawBackground(forGlyphRange: range, at: glyphOrigin)
        layoutManager.drawGlyphs(forGlyphRange: range, at: glyphOrigin)
    }
 */
    ...
    fileprivate func textOrigin(inRect rect: CGRect) -> CGPoint {
        ...
        if verticalTextAlignmentCenter {
            let usedRect = layoutManager.usedRect(for: textContainer)
            let glyphOriginY = (rect.height > usedRect.height) ? rect.origin.y + (rect.height - usedRect.height) / 2 : rect.origin.y
            glyphOrigin = CGPoint(rect.origin.x, glyphOriginY)
        }
